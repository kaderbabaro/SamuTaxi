...
class SignUpDriverForm extends StatefulWidget {
  const SignUpDriverForm({Key? key}) : super(key: key);

  @override
  State<SignUpDriverForm> createState() => _SignUpDriverFormState();
}

class _SignUpDriverFormState extends State<SignUpDriverForm> {
  ...
  Future<void> _uploadUserInfo() async {
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        final int id = responsData['id'];

        print('idchauffeur : $id');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Informations chauffeur', style: TextStyle(color: Colors.white)),
